...
def decode_file():
	...
	# first line of this section holds 4 count bytes?? last two for groups of so many items??
	# looks like this is optional, but the question is what controls it. These are nog found in the rtl and gate files
	# It looks like the next bit is a fixed set of data of 1332 lines long
	# have to figure out how manu lines it needs to be
	for i in range(3):
		get_and_output_items()

	# Next line seems to be a standard line with encoded items
	# the count is in the line before?
	#get_items()
...
def main():
	...
	inputfile = open("/home/peter/Data/Anlogic_projects/pin_test/pin_test_rtl.db", "rt")
	outputfile = open("/home/peter/Data/Anlogic_projects/pin_test/pin_test_rtl.db.txt", "wt")
	decode_file()
	inputfile.close()
	outputfile.close()
